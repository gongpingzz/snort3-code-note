    ...
    do
    {
        ...
        if ( node->otn )
        {
            ...
            if ( snort_protocol_id != UNKNOWN_PROTOCOL_ID )
            {
                ...
                {
                    if ( snort_protocol_id == svc.snort_protocol_id ) //数据包与规则中的snort_protocol_id对比 
                    {
                        check_ports = 0;
                        break;  // out of for
                    }
                ...
                if ( !sig_info.services.empty() and check_ports )//服务不能为空，服务是什么？
                {
        ...
        case RULE_OPTION_TYPE_FLOWBIT:
            if ( node->evaluate )
            {
                ...
                if ( flowbits_setoperation )
                    ...
                    rval = node->evaluate(node->option_data, cursor, eval_data.p); //fp_eval_option()
            }
            break;
