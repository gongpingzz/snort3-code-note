...
void LuaDetectorManager::load_detector(char* detector_filename, bool is_custom, bool reload, std::string& buf)
{
    ...
    {
        if (luaL_loadfile(L, detector_filename)) //加载lua文件，加载function元素到栈顶，但是不执行
        {
            if (init(L))
                ErrorMessage("Error - appid: can not load Lua detector, %s\n", lua_tostring(L, -1));
            lua_pop(L, 1);
    ...
    if (lua_object)
        allocated_objects.push_front(lua_object); // 创建检测器对象放入一个链表中
}
...
void LuaDetectorManager::load_lua_detectors(const char* path, bool is_custom, bool reload)
{
    ...
    if (rval == 0 )
    {
        if (lua_gettop(L)) //获取栈顶元素索引，空栈返回0
            WarningMessage("appid: leak of %d lua stack elements before detector load\n",
                lua_gettop(L));

        std::string buf;
        for (unsigned n = 0; n < globs.gl_pathc; n++) // 加载/usr/local/snort/appid/odp/lua下的检测器
        {
...
void LuaDetectorManager::activate_lua_detectors()
{
    ...
    while (lo != allocated_objects.end())
    {
        ...
        if (!lua_isfunction(L, -1)) //如果指定索引对应的值是函数，则返回1，其他返回0
        {
            if (init(L)) // 这里调用的是Lua脚本中的DetectorInit？
        ...
        {
            if (init(L)) //运行lua中的DetectorInit函数？
