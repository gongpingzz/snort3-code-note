    ...
    AppidSessionDirection dir, AppidChangeBits& change_bits)
{
    ...
    else if (!got_brute_force)
    {
        ...
        {
            get_next_service(p, dir, asd); //  切换下一个服务 port->patterns，在这里找到解析器了
        }
        ...
        {
            ServiceDetector* service = (ServiceDetector*)*it; //在这里就已经找到对应的解析器了
            int result;
    ...
    /* Failed all candidates, or no detector identified after seeing bidirectional exchange双向交换 */
