...
static void main_loop()
{
    ...
    {
        for (unsigned i = 0; i < max_pigs; i++) /* max_pigs 表示线程数 */
        {
    ...
    while ( swine or paused or (Trough::has_next() and !exit_requested) )
    {
        ...
        if ( idx >= 0 )
        {
            ...
            if ( pig.analyzer )
            {
                handle(pig, swine, pending_privileges); /* 从主线程切换到其他线程，执行处理数据包 */
...
static void snort_main()
{
    ...
    pig_poke = new Ring<unsigned>((max_pigs*max_grunts)+1); //此处有一个环的数据结构？做何用？
